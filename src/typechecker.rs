...
pub fn typecheck(program: Program) -> Result<(), (String, proc_macro2::Span)> {
    ...
    for item in program.items.iter() {
        match item {
            ProgramItem::Predicate(predicate) => {
                predicates.insert(predicate.name.to_string(), predicate);
            }
            ProgramItem::Rule(Rule { head, body }) => {
                ...
                if let Some(ref predicate) = predicates.get(&predicate_name) {
                    match head {
                        Atom::Positional { args, .. } => {
                            if args.len() != predicate.parameters.len() {
                                Err((
                                    format!(
                                        "predicate {} expects {} parameters, {} found",
                                        predicate_name,
                        ...
                        Atom::Named { .. } => {
                            Err((
                                "a predicate name must not use named bindings".to_string(),
                                head.predicate().span(),
                            ))?;
                        }
                    }
                    ...
                    Err((
                        format!("predicate {} is not declared", predicate_name),
                        head.predicate().span(),
                    ))?;
                }
            }
